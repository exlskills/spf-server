...
export async function viewCourseCertificate(client: GqlApi, user: IUserData, locale: string, courseGID: string) : Promise<ISPFRouteResponse> {
    ...
    let checkoutItem = {
        ...
        refs: {
            course_id: fromGlobalId(courseGID)
        },
        displayUnitCost: gqlResp.meta.verified_cert_cost,
        displayName: gqlResp.meta.title + ' Verified Certificate'
    };
    return {
        contentTmpl: 'course_certificate',
        meta: {
            title: gqlResp.meta.title + ' Certificate',
